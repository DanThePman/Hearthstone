...
namespace HearthstoneMulligan.Specific_Mulligans
{
    class DeckTypeDetector
    {
        ...
        public static DeckType SearchForDeckType()
        {
            try
            {
                ...
                var deckCards = MainLists.currentDeck.Cards.ToArray();

                //cards are probably doubled included
                if (deckCards.Count(MECH_Cards.Contains) >= 7)
                {
                    thisDeckType = DeckType.MECH;
                }
                else if (deckCards.Count(OTK_WARRIOR_Cards.Contains) >= 10 &&
                    MainLists.OwnClass == Card.CClass.WARRIOR &&
                        deckCards.All(x => x.ToString() != "BRM_019"))//Grim Patron
                {
                    thisDeckType = DeckType.OTK_WARRIOR;
                }
                else if (deckCards.Count(MIRACLE_ROGUE_Cards.Contains) >= 10 && 
                    MainLists.OwnClass == Card.CClass.ROGUE &&
                        deckCards.Any(x => x.ToString() == "EX1_095"))//Gadgetzan Auctioneer
                {
                    thisDeckType = DeckType.MIRACLE_ROGUE;
                }
                else if (deckCards.Count(PIRATE_ROGUE_Cards.Contains) >= 10)
                {
                    thisDeckType = DeckType.PIRAT_ROGUE;
                }
                else if (deckCards.Count(MURLOC_Cards.Contains) >= 10)
                {
                    thisDeckType = DeckType.MURLOC;
                }
                else if (deckCards.Count(HANDLOCK_WARLOCK_Cards.Contains) >= 10)
                {
