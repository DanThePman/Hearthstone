...
namespace SmartBotUI.Mulligan
{
    [Serializable]a
        ...
        public class ValueReader
        {
            ...
            {
                /// <summary>
                /// Only add combos if cards have'nt a bad effect
                /// </summary> 
                public static bool OnlyComboCheckIfCardHasNoBadEffect(Card MulliganCard)
                {
                    return !ValueReader.ValueIgnorer.MakeIgnoringOnlyIfNotBadEffect
                        ||
                        (ValueReader.ValueIgnorer.MakeIgnoringOnlyIfNotBadEffect &&
                        !CardProperties.HasBadEffect(MulliganCard));
                }
                ...
                public static bool HandContains2234(List<Card> HandCards)
                {
                    ...
                    return
                        ...
                        x.Cost == 2 && OnlyComboCheckIfCardHasNoBadEffect(x)) >= 2
                        &&
                        HandCards.Any(x => new NeutralMinion(x).BoardCard != null && x.Cost == 3 &&
                            OnlyComboCheckIfCardHasNoBadEffect(x))
                        &&
                        HandCards.Any(x => new NeutralMinion(x).BoardCard != null && x.Cost == 4 &&
                            OnlyComboCheckIfCardHasNoBadEffect(x));
                    }

                public static bool HandContains224(List<Card> HandCards)
                {
                    if (!HandCards.Any(x => x.Name.Equals("GAME_005"))) //Coin
                        return false;
                    ...
                    return
                        ...
                        HandCards.Any(x => new NeutralMinion(x).BoardCard != null &&
                            x.Cost == 4 && OnlyComboCheckIfCardHasNoBadEffect(x));
                    }
                ...
                public static IOrderedEnumerable<KeyValuePair<ComboPriorities, int>> AllComboCasesSortedByPriority
                {
                    get
                    {
            ...
            : base()
        {
            ...
            Combos.alreadyFoundOneCombo = false;
        }
        ...
        private void CaseThree(int maxMana, List<Card> HandCards)
        {
            if (ValueReader.ValueIgnorer.IgnoreValueIf_244_AndCoin &&
                ValueReader.ValueIgnorer.HandContains224(HandCards) &&
                !Combos.alreadyFoundOneCombo)
            {
                ...
                if (bestDrops.Count(x => ValueReader.ValueIgnorer.OnlyComboCheckIfCardHasNoBadEffect(x)) ==
                        bestDrops.Length)
                {
                    foreach (var drop in bestDrops.Where(x => !chosenCards.Contains(x)))
                    {
                        chosenCards.Add(drop);
                        blackList.Add(drop.Name);
                    }
                }
            }
        }

        private void CaseTwo(int maxMana, List<Card> HandCards)
        {
            if (ValueReader.ValueIgnorer.IgnoreValueIf_2234_AndCoin &&
                ValueReader.ValueIgnorer.HandContains2234(HandCards) &&
                !Combos.alreadyFoundOneCombo)
            {
                Combos.alreadyFoundOneCombo = true;
                var best2Drop =
        ...
        private void CaseOne(int maxMana, List<Card> HandCards)
        {
            ...
            if (ValueReader.ValueIgnorer.IgnoreValueIfCardIsX_DropEtc
                &&
                HandCards.Any(x => new NeutralMinion(x).BoardCard != null && x.Cost == X_Config_Drop &&
                    ValueReader.ValueIgnorer.OnlyComboCheckIfCardHasNoBadEffect(x))
                &&
                HandCards.Any(x => new NeutralMinion(x).BoardCard != null && x.Cost == X_Config_Drop - 1 &&
                    ValueReader.ValueIgnorer.OnlyComboCheckIfCardHasNoBadEffect(x))
                &&
                HandCards.Any(x => new NeutralMinion(x).BoardCard != null && x.Cost == X_Config_Drop - 2 &&
                    ValueReader.ValueIgnorer.OnlyComboCheckIfCardHasNoBadEffect(x))
                && !Combos.alreadyFoundOneCombo)
            {
                Combos.alreadyFoundOneCombo = true;
        ...
        private void ManageCombos(int maxMana, List<Card> HandCards)
        {
            #region comboChecks
